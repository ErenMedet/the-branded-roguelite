...
- A currency is paid out of what the encounter cost, never per kill: the spawner buys a pack with threat and hands that pack's purse to its members (`WaveSpawner._ashPerThreat`), so the prefab's own reward is only a fallback for enemies placed by hand. Per-kill rewards make the cheapest swarm the best farm — six one-hit spirits worth 3 ash each out-earn a troll by ten to one — and anything spawned for free afterwards (a cultist's revived escort) becomes an endless tap, so it is worth nothing.
